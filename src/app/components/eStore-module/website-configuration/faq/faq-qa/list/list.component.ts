...
export class ListComponent implements OnInit {
  ...
  getFaqCategoryData() {
    ...
    this.httpService.getMethod('api/v2/website/faq/institute/' + sessionStorage.getItem('institute_id'), null).subscribe(
      (res: any) => {
  ...
  editRow(object) {
    ...
    this.router.navigate(['view/e-store/website-configuration/faq/qa/edit/' + object.data.id])
  }

  deleteRow(obj) {
    this.auth.showLoader();
    this.httpService.deleteFile('/prod/api/v2/website/faq/' + obj.data.id).subscribe(
      (res: any) => {
        this.auth.hideLoader();
        this.msgService.showErrorMessage('success','','FAQ Deleted Successfully');
        this.getFaqCategoryData();
      },
      err => {
        this.auth.hideLoader();
