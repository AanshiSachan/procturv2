...
export class EditClassComponent implements OnInit {
  ...
  getEvent(event) {
    ...
    if(new Date(proctur_live_expiry_date)<new Date(event) && new Date(proctur_live_expiry_date)!=new Date(event)){
      const tempMsg = 'Your live class subscription got expired on '.concat(proctur_live_expiry_date).concat(' hence you will not be able create live class! Renew your subscription to conduct live classes again!');      this.msgService.showErrorMessage('info','' , tempMsg);
